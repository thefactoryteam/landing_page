...
const PartnerSection = () => {
  return (
    <div className="w-full md:w-4/5 mx-auto mt-10">
      {/* Partners Section */}
      <div className="">
        <div className="bg-black text-white md:rounded-2xl px-5 md:px-10 py-20">
          <h2 className="text-xl md:text-2xl font-bold">Our Innovation Champions (Partners)</h2>
          <p className="text-gray-300 text-lg md:text-xl mt-2 md:w-[800px]">
            ...
            >
              {partners.map((logo, index) => (
                <div key={index} className="mx-8 flex items-center">
                  <img src={logo} alt={`partner-${index}`} className="h-6 md:h-8 object-contain" />
                </div>
              ))}
            </Marquee>
          </div>
          ...
          {/* Partner Button */}
          <motion.button
            className="mt-5 bg-[#2BE028] hover:text-white cursor-pointer px-6 py-2 text-black font-semibold rounded-full"
          >
            Partner with Us
          </motion.button>
        </div>
